...
class Raytracer {
    ...
    private:
        ...
        vec3 shade(vec3 rayDir, vec3 lightDir, vec3 intersectionPoint, vec3 normal, Light *light, Material const& material);
        vec3 shade_BDSM(vec3 rayDir, vec3 lightDir, vec3 intersectionPoint, vec3 normal, Light *light, Material const& material);
        ...
        vec3 RDM_bsdf(float LdotH, float NdotH, float VdotH, float LdotN, float VdotN, Material const& m);
        vec3 RDM_bsdf_d(Material const& m);
